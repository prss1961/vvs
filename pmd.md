...
```
### 5. UseArraysAsList (Performace)

@paulosilveira A classe java.util.Arrays tem um método "asList" que deve ser usado quando você deseja criar uma nova Lista a partir de uma matriz de objetos. É mais rápido do que executar um loop para copiar todos os elementos do array um por um.
Observe que o resultado de Arrays.asList () é apoiado pela matriz especificada, as alterações na lista retornada resultarão na modificação da matriz. Por esse motivo, não é possível adicionar novos elementos à lista retornada de Arrays.asList () (UnsupportedOperationException). Você deve usar new ArrayList <> (Arrays.asList (…)) se isso for inconveniente para você (por exemplo, por causa de acesso simultâneo).
...
### 6. AvoidProtectedFieldInFinalClass (Code Style)   

@paulosilveira Não use campos protegidos em classes finais, pois eles não podem ser subclasses. Esclareça sua intenção usando modificadores de acesso privado ou de pacote.
